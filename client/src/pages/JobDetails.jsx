...
const JobDetails = () => {
    ...
    const getJobTypeBadge = (type) => {
        const c = { 'full-time': 'bg-violet-100 text-violet-700 dark:bg-violet-900/40 dark:text-violet-300', 'part-time': 'bg-sky-100 text-sky-700 dark:bg-sky-900/40 dark:text-sky-300', 'contract': 'bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-300', 'internship': 'bg-violet-100 text-violet-700 dark:bg-violet-900/40 dark:text-violet-300', 'remote': 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300' };
        return c[type] || 'bg-gray-100 text-gray-800';
    };

    const getGradient = (char) => {
        const g = ['from-violet-500 to-indigo-600', 'from-amber-500 to-orange-600', 'from-sky-500 to-blue-600', 'from-rose-500 to-pink-600', 'from-violet-500 to-purple-600', 'from-lime-500 to-green-600'];
    ...
    return (
        <div className="page-container min-h-screen" style={{ background: 'var(--bg-tertiary)' }}>
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <button onClick={() => navigate(-1)} className="flex items-center gap-2 font-medium mb-6 transition-colors duration-300 hover:text-violet-600 slide-in-left" style={{ color: 'var(--text-secondary)' }}><FaArrowLeft /> Back to Jobs</button>
                        ...
                        <div className="flex flex-col md:flex-row md:items-start gap-6">
                            ...
                            <div className="flex-1">
                                ...
                                <div className="flex flex-wrap gap-5" style={{ color: 'var(--text-muted)' }}>
                                    ...
                                    <span className="flex items-center gap-2 text-sm"><FaClock className="text-violet-500" />Posted {formatDate(job.createdAt)}</span>
                                </div>
                            </div>
                            <div className="flex-shrink-0">
                                {!isAuthenticated ? <Link to="/login" className="btn btn-primary btn-ripple">Login to Apply</Link>
                                    : applied ? <button disabled className="btn bg-violet-500 text-white flex items-center gap-2 shadow-lg shadow-violet-500/25 bounce-in"><FaCheckCircle /> Applied</button>
                                ...
                                <div className="card p-6">
                                    ...
                                    <ul className="space-y-3">{job.requirements.map((req, i) => (
                                        <li key={i} className="flex items-start gap-3 text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
                                            <span className="w-2 h-2 bg-gradient-to-r from-violet-500 to-indigo-500 rounded-full mt-2 flex-shrink-0"></span>{req}
                                        </li>))}
                                    </ul>
                                </div>
                            </AnimatedSection>
                            ...
                            <AnimatedSection animation="slide-in-right" delay={150}>
                                <div className="card p-6">
                                    ...
                                    <div className="flex flex-wrap gap-2">{job.skills.map((skill, i) => (
                                        <span key={i} className="px-3 py-1.5 bg-violet-50 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300 rounded-lg text-sm font-medium transition-all duration-300 hover:scale-105 hover:shadow-md cursor-default">{skill}</span>))}
                                    </div>
                                </div>
                            </AnimatedSection>
                        )}
                    ...
                    >
                        ...
                        <form onSubmit={handleApply} className="space-y-5">
                            <div>
                                ...
                                <div className="border-2 border-dashed rounded-xl p-6 text-center transition-all duration-300 hover:border-violet-500 group cursor-pointer" style={{ borderColor: 'var(--input-border)' }}>
                                    ...
                                    <label htmlFor="resume-upload" className="cursor-pointer">
                                        ...
                                        {resume ? <p className="text-violet-600 dark:text-violet-400 font-bold">{resume.name}</p> : <p style={{ color: 'var(--text-muted)' }}>Click to upload your resume</p>}
                                    </label>
                                </div>
                            </div>
                            <div>
