...
const JobCard = ({ job }) => {
    ...
    const getJobTypeBadge = (type) => {
        const colors = {
            'full-time': 'bg-violet-100 text-violet-700 dark:bg-violet-900/40 dark:text-violet-300',
            ...
            'internship': 'bg-rose-100 text-rose-700 dark:bg-rose-900/40 dark:text-rose-300',
    ...
    return (
        <motion.div
            className="card card-tilt p-6 group"
        ...
        >
            <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h3 className="font-bold text-lg text-gray-900 dark:text-white group-hover:text-violet-600 dark:group-hover:text-violet-400 transition-colors duration-300">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
                {job.skills?.slice(0, 3).map((skill, index) => (
                    <span key={index} className="px-2.5 py-1 bg-violet-50 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300 text-xs rounded-lg font-medium">{skill}</span>
                ))}
                {job.skills?.length > 3 && (
            ...
            <div className="flex flex-wrap gap-4 text-sm text-gray-500 dark:text-gray-400 mb-4">
                <span className="flex items-center gap-1.5"><FaMapMarkerAlt className="text-violet-500 text-xs" />{job.location}</span>
                <span className="flex items-center gap-1.5"><FaBriefcase className="text-violet-500 text-xs" />{job.experience}</span>
