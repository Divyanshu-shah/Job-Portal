            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between items-center h-16">
                    ...
                    <Link to="/" className="flex items-center gap-2.5 text-2xl font-bold group">
                        <div className="w-9 h-9 bg-gradient-to-br from-violet-500 to-indigo-600 rounded-xl flex items-center justify-center text-white shadow-lg group-hover:shadow-violet-500/30 transition-all duration-400 group-hover:scale-105">
                    ...
                    <div className="hidden md:flex items-center gap-1">
                        ...
                        {isAuthenticated ? (
                            <>
                                ...
                                <div className="relative ml-4">
                                    ...
                                    >
                                        <div className="w-8 h-8 bg-gradient-to-br from-violet-500 to-indigo-600 rounded-full flex items-center justify-center text-white text-sm font-bold shadow-inner">
                        ...
                        <button onClick={toggleTheme} className="theme-toggle ml-4" aria-label="Toggle theme">
                            {isDark ? <FaSun className="text-xl text-amber-400" /> : <FaMoon className="text-xl text-violet-700" />}
                        </button>
                    </div>
                    ...
                    <div className="md:hidden flex items-center gap-2">
                        <button onClick={toggleTheme} className="theme-toggle" aria-label="Toggle theme">
                            {isDark ? <FaSun className="text-lg text-amber-400" /> : <FaMoon className="text-lg text-violet-700" />}
                        </button>
            ...
            {isOpen && (
                <div className="md:hidden fade-in backdrop-blur-xl" style={{ backgroundColor: 'var(--card-bg)', borderTopColor: 'var(--border-color)', borderTopWidth: '1px' }}>
                    <div className="px-4 py-4 space-y-1">
                        <Link to="/" onClick={() => setIsOpen(false)} className="block px-4 py-3 rounded-xl font-medium transition-all hover:bg-violet-50 dark:hover:bg-violet-900/20" style={{ color: 'var(--text-primary)' }}>Home</Link>
                        <Link to="/jobs" onClick={() => setIsOpen(false)} className="block px-4 py-3 rounded-xl font-medium transition-all hover:bg-violet-50 dark:hover:bg-violet-900/20" style={{ color: 'var(--text-primary)' }}>Browse Jobs</Link>
                        {isAuthenticated ? (
                            <>
                                <Link to={getDashboardLink()} onClick={() => setIsOpen(false)} className="block px-4 py-3 rounded-xl font-medium transition-all hover:bg-violet-50 dark:hover:bg-violet-900/20" style={{ color: 'var(--text-primary)' }}>Dashboard</Link>
